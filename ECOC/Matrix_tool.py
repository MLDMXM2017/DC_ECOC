...
def select_column(M,data,label,label_length):
    ...
    for i,each in enumerate(np.unique(label)):
        ...
        cplx[each] = ECOCDemo.DC.Get_Complexity.get_complexity_D2(cls_data,cls_label,k=5)
    bottom_stand = np.mean(cplx.values())
    cplx_stand = np.mean(cplx.values()) * 2

    logging.info("class cplx:\r\n" + str(cplx))
    logging.info("var:%-10f mean:%-10f" %(np.var(cplx.values()), np.mean(cplx.values())))
    ...
    for i, each in enumerate(np.unique(label)):
        ...
        if cplx[each] > cplx_stand:
            cls_num[each] = 2
    column_num = label_length + np.sum(cls_num.values())

    logging.info("column num:%d" %np.sum(cls_num.values()))
    ...
    if len(M[0]) < column_num:
        ...
        for i in range(column_num -  len(M[0])):#add column_num-M column
...
def remove_reverse(M):
    ...
    for i in range(len(M[0])):
        for j in range(i+1,len(M[0])):
            ...
            if cmp(i_column,list(-np.array(j_column))) == 0:
        ...
        if (1 not in column or -1 not in column) and column.count(0)  != len(column):#non all 0
            ...
            for j, each in enumerate(np.unique(label)):
                if j in class_to_change:
                    cls_data, cls_label = get_data_subset(data, label, each)
                    cplx[j] = ECOCDemo.DC.Get_Complexity.get_complexity_D2(cls_data, cls_label, k=5)
